...
func buildDatabaseClientForTest(t *testing.T) (*repository, audit.Repository) {
	...
	require.NoError(t, err)
	c := ProvideIssueReportsRepository(loggingnoop.NewLogger(), tracingnoop.NewTracerProvider(), auditLogEntryRepo, pgc, nil)
	require.NoError(t, err)
...
func buildInertClientForTest(t *testing.T) *repository {
	...
	c := ProvideIssueReportsRepository(loggingnoop.NewLogger(), tracingnoop.NewTracerProvider(), nil, &mockdatabase.ClientMock{ReaderFunc: func() database.SQLQueryExecutor { return nil }, WriterFunc: func() database.SQLQueryExecutor { return nil }}, nil)
