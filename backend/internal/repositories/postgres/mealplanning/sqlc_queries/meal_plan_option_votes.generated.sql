...
SELECT
	...
	(
		...
		WHERE meal_plan_option_votes.archived_at IS NULL
			AND
			meal_plan_option_votes.created_at > COALESCE(sqlc.narg(created_after), (SELECT NOW() - '999 years'::INTERVAL))
			...
			AND (NOT COALESCE(sqlc.narg(include_archived), false)::boolean OR meal_plan_option_votes.archived_at IS NULL)
			AND meal_plan_option_votes.belongs_to_meal_plan_option = sqlc.arg(meal_plan_option_id)
	) AS filtered_count,
	(
		SELECT COUNT(meal_plan_option_votes.id)
