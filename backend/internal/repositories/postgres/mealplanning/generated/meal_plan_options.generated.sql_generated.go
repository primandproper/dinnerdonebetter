...
// versions:
//   sqlc v1.25.0
// source: meal_plan_options.generated.sql
...
const checkMealPlanOptionBelongsToAccount = `-- name: CheckMealPlanOptionBelongsToAccount :one
SELECT EXISTS (
	SELECT meal_plan_options.id
	FROM meal_plan_options
...
type CheckMealPlanOptionBelongsToAccountParams struct {
	MealPlanOptionID string
	BelongsToAccount string
}

func (q *Queries) CheckMealPlanOptionBelongsToAccount(ctx context.Context, db DBTX, arg *CheckMealPlanOptionBelongsToAccountParams) (bool, error) {
	row := db.QueryRowContext(ctx, checkMealPlanOptionBelongsToAccount, arg.MealPlanOptionID, arg.BelongsToAccount)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
SELECT
	...
	(
		...
		FROM meal_plan_options
		WHERE meal_plan_options.created_at > COALESCE($1, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			AND meal_plan_options.created_at < COALESCE($2, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			AND (
				meal_plan_options.last_updated_at IS NULL
				OR meal_plan_options.last_updated_at > COALESCE($3, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			)
			AND (
				meal_plan_options.last_updated_at IS NULL
				OR meal_plan_options.last_updated_at < COALESCE($4, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			)
