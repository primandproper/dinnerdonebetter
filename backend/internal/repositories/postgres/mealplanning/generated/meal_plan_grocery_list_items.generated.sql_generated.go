...
// versions:
//   sqlc v1.25.0
// source: meal_plan_grocery_list_items.generated.sql
...
SELECT
	...
	(
		...
		JOIN valid_ingredients ON meal_plan_grocery_list_items.valid_ingredient = valid_ingredients.id
		JOIN valid_measurement_units ON meal_plan_grocery_list_items.valid_measurement_unit = valid_measurement_units.id
		WHERE meal_plan_grocery_list_items.created_at > COALESCE($1, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			AND meal_plan_grocery_list_items.created_at < COALESCE($2, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			AND (
				meal_plan_grocery_list_items.last_updated_at IS NULL
				OR meal_plan_grocery_list_items.last_updated_at > COALESCE($3, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			)
			AND (
				meal_plan_grocery_list_items.last_updated_at IS NULL
				OR meal_plan_grocery_list_items.last_updated_at < COALESCE($4, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			)
