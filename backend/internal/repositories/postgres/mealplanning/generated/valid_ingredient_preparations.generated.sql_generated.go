...
// versions:
//   sqlc v1.25.0
// source: valid_ingredient_preparations.generated.sql
...
SELECT
	...
	(
		...
		WHERE valid_ingredient_preparations.archived_at IS NULL
			...
			AND (NOT COALESCE($5, false)::boolean OR valid_ingredient_preparations.archived_at IS NULL)
	) AS filtered_count,
	(
		SELECT COUNT(valid_ingredient_preparations.id)
		FROM valid_ingredient_preparations
...
const getValidIngredientPreparationsByIDs = `-- name: GetValidIngredientPreparationsByIDs :many
SELECT
	valid_ingredient_preparations.id as valid_ingredient_preparation_id,
	valid_ingredient_preparations.notes as valid_ingredient_preparation_notes,
	valid_preparations.id as valid_preparation_id,
...
WHERE
	...
	AND valid_ingredients.archived_at IS NULL
	AND valid_preparations.archived_at IS NULL
	AND valid_ingredient_preparations.id = ANY($1::text[])
`

type GetValidIngredientPreparationsByIDsRow struct {
	ValidIngredientPreparationID                           string
	ValidIngredientPreparationNotes                        string
	ValidPreparationID                                     string
	ValidPreparationName                                   string
...
func (q *Queries) GetValidIngredientPreparationsByIDs(ctx context.Context, db DBTX, ids []string) ([]*GetValidIngredientPreparationsByIDsRow, error) {
	rows, err := db.QueryContext(ctx, getValidIngredientPreparationsByIDs, pq.Array(ids))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []*GetValidIngredientPreparationsByIDsRow{}
	for rows.Next() {
		var i GetValidIngredientPreparationsByIDsRow
		if err := rows.Scan(
			&i.ValidIngredientPreparationID,
			&i.ValidIngredientPreparationNotes,
			&i.ValidPreparationID,
