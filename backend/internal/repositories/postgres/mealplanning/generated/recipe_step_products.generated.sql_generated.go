...
// versions:
//   sqlc v1.25.0
// source: recipe_step_products.generated.sql
...
type CreateRecipeStepProductParams struct {
	...
	MinimumMeasurementQuantityValue    sql.NullString
	MaximumMeasurementQuantityValue    sql.NullString
	MinimumItemQuantityValue           sql.NullString
	MaximumItemQuantityValue           sql.NullString
...
func (q *Queries) CreateRecipeStepProduct(ctx context.Context, db DBTX, arg *CreateRecipeStepProductParams) error {
	_, err := db.ExecContext(ctx, createRecipeStepProduct,
		...
		arg.Type,
		arg.MeasurementUnit,
		arg.MinimumMeasurementQuantityValue,
		arg.MaximumMeasurementQuantityValue,
		arg.MinimumItemQuantityValue,
		arg.MaximumItemQuantityValue,
		arg.QuantityNotes,
		arg.Compostable,
...
func (q *Queries) GetRecipeStepProduct(ctx context.Context, db DBTX, arg *GetRecipeStepProductParams) (*GetRecipeStepProductRow, error) {
	...
	err := row.Scan(
		...
		&i.MinimumMeasurementQuantityValue,
		&i.MaximumMeasurementQuantityValue,
		&i.MinimumItemQuantityValue,
		&i.MaximumItemQuantityValue,
		&i.QuantityNotes,
		&i.Compostable,
...
SELECT
	...
	(
		...
		FROM recipe_step_products
		WHERE recipe_step_products.created_at > COALESCE($1, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			AND recipe_step_products.created_at < COALESCE($2, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			AND (
				recipe_step_products.last_updated_at IS NULL
				OR recipe_step_products.last_updated_at > COALESCE($3, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			)
			AND (
				recipe_step_products.last_updated_at IS NULL
				OR recipe_step_products.last_updated_at < COALESCE($4, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			)
			AND (COALESCE($5, false)::boolean OR recipe_step_products.archived_at IS NULL)
	) AS filtered_count,
	(
		SELECT COUNT(recipe_step_products.id)
		FROM recipe_step_products
...
WHERE recipe_step_products.created_at > COALESCE($1, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
	...
	AND recipe_step_products.belongs_to_recipe_step = $6
	AND recipe_steps.archived_at IS NULL
	AND recipe_steps.id = $6
	AND recipe_steps.belongs_to_recipe = $7
	AND recipes.archived_at IS NULL
	AND recipes.id = $7
	AND recipe_step_products.id > COALESCE($8, '')
...
UPDATE recipe_step_products SET
	...
	type = $2,
	measurement_unit = $3,
	minimum_measurement_quantity_value = $4,
	maximum_measurement_quantity_value = $5,
	minimum_item_quantity_value = $6,
	maximum_item_quantity_value = $7,
	quantity_notes = $8,
	compostable = $9,
	maximum_storage_duration_in_seconds = $10,
	minimum_storage_temperature_in_celsius = $11,
	maximum_storage_temperature_in_celsius = $12,
