...
// versions:
//   sqlc v1.25.0
// source: valid_ingredient_measurement_units.generated.sql
...
SELECT
	...
	(
		...
		FROM valid_ingredient_measurement_units
		WHERE valid_ingredient_measurement_units.created_at > COALESCE($1, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			AND valid_ingredient_measurement_units.created_at < COALESCE($2, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			AND (
				valid_ingredient_measurement_units.last_updated_at IS NULL
				OR valid_ingredient_measurement_units.last_updated_at > COALESCE($3, (SELECT CURRENT_TIMESTAMP - '999 years'::INTERVAL))
			)
			AND (
				valid_ingredient_measurement_units.last_updated_at IS NULL
				OR valid_ingredient_measurement_units.last_updated_at < COALESCE($4, (SELECT CURRENT_TIMESTAMP + '999 years'::INTERVAL))
			)
...
const getValidIngredientMeasurementUnitsByIDs = `-- name: GetValidIngredientMeasurementUnitsByIDs :many
SELECT
	valid_ingredient_measurement_units.id as valid_ingredient_measurement_unit_id,
	valid_ingredient_measurement_units.notes as valid_ingredient_measurement_unit_notes,
	valid_measurement_units.id as valid_measurement_unit_id,
...
WHERE
	...
	AND valid_measurement_units.archived_at IS NULL
	AND valid_ingredients.archived_at IS NULL
	AND valid_ingredient_measurement_units.id = ANY($1::text[])
`
...
func (q *Queries) GetValidIngredientMeasurementUnitsByIDs(ctx context.Context, db DBTX, ids []string) ([]*GetValidIngredientMeasurementUnitsByIDsRow, error) {
	rows, err := db.QueryContext(ctx, getValidIngredientMeasurementUnitsByIDs, pq.Array(ids))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []*GetValidIngredientMeasurementUnitsByIDsRow{}
	for rows.Next() {
		var i GetValidIngredientMeasurementUnitsByIDsRow
		if err := rows.Scan(
			&i.ValidIngredientMeasurementUnitID,
			&i.ValidIngredientMeasurementUnitNotes,
			&i.ValidMeasurementUnitID,
