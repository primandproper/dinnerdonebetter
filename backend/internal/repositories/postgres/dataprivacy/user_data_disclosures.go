...
func (r *disclosureRepository) CreateUserDataDisclosure(ctx context.Context, input *dataprivacy.UserDataDisclosureCreationInput) (*dataprivacy.UserDataDisclosure, error) {
	...
	// One transaction, for the reason the audit package takes a querier at all: the
	// record of a disclosure request that survived a rolled-back request would be a
	// record of something that did not happen. It is also what gives Record's lock
	// on the chain head a transaction to be held for.
	if err := r.WithTransaction(ctx, func(tx database.SQLQueryExecutor) error {
		...
		return r.auditLogRepo.Record(ctx, tx, &audit.Entry{
...
func (r *disclosureRepository) ArchiveUserDataDisclosure(ctx context.Context, disclosureID string) error {
	...
	if err = r.WithTransaction(ctx, func(tx database.SQLQueryExecutor) error {
		...
		if err = r.auditLogRepo.Record(ctx, tx, &audit.Entry{
			ResourceType: resourceTypeUserDataDisclosure,
			ResourceID:   disclosureID,
			EventType:    audit.EventArchived,
			Actor:        audit.UserActor(disclosure.BelongsToUser),
		}); err != nil {
