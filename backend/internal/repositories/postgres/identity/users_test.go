...
func TestQuerier_Integration_Users(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = dbc.writeDB.ExecContext(ctx, "INSERT INTO user_role_assignments (id, user_id, role_name) VALUES ($1, $2, $3)", identifiers.New(), firstUser.ID, authorization.ServiceAdminRoleName)
	require.NoError(t, err)
