...
🟡 **H10 — Every user's meal lists and meal-plan sub-resources are cross-tenant readable/mutable.** `internal/services/mealplanning/grpc/meal_planning.go:230-256,339-367` (lists) and the sub-resource handlers at `:90,112,135,159,478,503,758,783,810,835,1004,1030,1057,1085,1112,1137` (events, options, votes, grocery items, tasks, selections). These skip session/account scoping while their parent handlers scope by active account and the managers never check the account. *Fix:* thread the session active-account ID through, matching the meal-plan-level handlers. **Done:** meal-plan sub-resources (events/options/votes/grocery/tasks) now guard via `verifyMealPlanAccess`. **Deferred:** `GetMealLists`/`GetMealListItems` and the recipe-option-selection handlers can't be service-scoped without a manager-signature change + a repository `belongs_to_user` predicate (`meal_lists.go:43`, codegen `mealplanning_meal_lists.go:79-96`) — still open.
...
- **H10 / H18 tail** — the meal-list, recipe-option-selection, and vote-eligibility scoping that needs a manager-signature change + repository `belongs_to_user` predicates.
