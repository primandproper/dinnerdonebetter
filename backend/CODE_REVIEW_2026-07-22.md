...
- **H10 / H18 tail** — meal-list scoping is now done (`belongs_to_user` predicate + session threading); what remains is the recipe-option-selection scoping (needs an option→account resolution query) and the H18 vote-eligibility gate (needs the meal-plan/event IDs threaded into the manager signature).
