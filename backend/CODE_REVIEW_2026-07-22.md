...
**Rollup:** High — 20 ✅, 2 🟡 (H15, H18). Medium — 25 ✅, 1 🟡 (M16), 6 ⏸️ (M5, M12, M13, M26, M29, M33), 1 ↩️ (M23). Root causes — RC1 ✅, RC2 🟡, RC3 ✅, RC4 ⏸️. Lows — mixed (marked per item). Verification: `go build ./...`, `go vet ./...`, unit tests, and `testing/integration/apiserver` all pass.
...
- ⏸️ **M5** `cmd/tools/codegen/queries/mealplanning_recipes.go:124-139,222-237` — `GetRecipeByID`/`GetRecipesWithIDs` LEFT JOIN `recipe_steps` with no `archived_at IS NULL`, so archived steps reappear in fetched recipes. *Fix:* add the filter to the join. **Not addressed.**
...
- ⏸️ **M12** `internal/domain/identity/converters/account_user_memberships.go:15` — converter sets `AccountID: account.ID` (the membership's own ID) instead of `account.BelongsToAccount`; latent, would create memberships pointing at nonexistent accounts. *Fix:* use `BelongsToAccount`. **Not addressed** (identity domain manager/converters were unassigned this pass).
- ⏸️ **M13** `internal/domain/identity/manager/user_data_manager.go:362-367` — `CreateUser` logs the live `InvitationToken` (a redemption secret) to log storage. *Fix:* log the invitation ID only. **Not addressed.**
...
- 🟡 **M16** `internal/domain/oauth/manager/manager.go:101` + `oauth2_client_info.go:20-22` + `revoke.go:72` — OAuth2 client secrets stored in plaintext and compared with `!=` (non-constant-time). *Fix:* store a hash, compare with `crypto/subtle.ConstantTimeCompare`. **Done:** constant-time comparison. **Deferred:** hashed storage (needs a migration + go-oauth2 store changes).
...
- ↩️ **M23** `internal/services/waitlists/grpc/waitlists.go:233,278-311` — `Get/Update/ArchiveWaitlistSignup` never compare `BelongsToUser`, so any user can read/mutate another's signup by ID. *Fix:* ownership check after load. **Reverted:** integration tests show signups are created by an admin and read by other users — they behave as admin/account-managed records, not user-private data. The user-ownership assumption was wrong. **Needs a product decision:** should signups be publicly readable by any authenticated user, or account-scoped (members only)?
...
- ⏸️ **M26** `internal/grpc/converters/query_filter.go:25` — `uint8(*qf.MaxResponseSize)` truncates the client's uint32 page size mod 256 (256→0, 300→44). *Fix:* clamp to a max or reject out-of-range with `InvalidArgument`. **Not addressed.**
...
- ⏸️ **M29** `internal/build/services/api/server.go:28-46` — two fully independent DI containers each build their own DB pool / MQ connections / observability stack, and with `runMigrations: true` in prod the migration runner executes twice per boot; neither `RootScope` is ever `Shutdown()`. *Fix:* share one injector (or at least the DB client + observability) and call shutdown on teardown. **Not addressed.**
...
- ⏸️ **M33** `cmd/services/mcp/main.go:44,143-178` — the MCP server hardcodes `:8888` and ignores `cfg.HTTPServer.Port` while configs say `8000` and validation requires the unused HTTP config. *Fix:* bind to the configured port or drop the field. **Not addressed.**
...
- ✅ `internal/domain/mealplanning/grocerylistpreparation/list_creator.go:167` — merging grocery items always sums min but only sums max when both sides have one, producing `min > max`. Treat a missing max as its min (or clear it). *(min/max fixed; the `:150` consolidated-item misattribution note remains open.)*
...
- ⏸️ `internal/domain/identity/manager/user_data_manager.go:704-724` — search-service branch of `SearchForUsers` ignores the filter and reports `len(users)` as both counts. **Not addressed.** ✅ `internal/domain/auth/managers/auth_manager.go:198` — TOTP verify bypasses the injected `totpVerifier` — **fixed** (as part of H3).

**Dropped/mis-mapped fields**
- ⏸️ `internal/domain/mealplanning/meal_plan_option.go:57` — `Selections` on option creation is converted nowhere and silently dropped. **Not addressed.**
...
- 🟡 `internal/services/mealplanning/grpc/meal_planning.go:625` (**fixed** → `codes.Internal`) and `internal/services/identity/grpc/admin.go:50` (**not addressed** — still `codes.Unauthenticated`, and it passes `nil` into `PrepareAndLogGRPCStatus`, so the permission check is also an RC1-style bypass returning `(nil, nil)`; worth fixing).
...
- ⏸️ `internal/domain/identity/manager/user_data_manager.go:40` (`todoRes = "TODO"`), `internal/domain/identity/converters/users.go:8-16` (`ConvertUserToUserCreationResponse`, drops fields). **Not addressed.**
- ⏸️ `internal/domain/mealplanning/meal.go:274` — `Meal`/`MealComponent` update types + methods are unreachable (no update op in the interface). **Not addressed.**
...
- ⏸️ Nil-input guards missing (inconsistent with sibling managers): `dataprivacy/manager/manager.go:59`, `uploadedmedia/manager/manager.go:65`, `waitlists/manager/manager.go:104,178`, `issuereports/manager/manager.go:105`, `webhooks/manager/webhook_manager.go:237`. **Not addressed.**

**Security hygiene (low)**
- ⏸️ `internal/services/auth/handlers/authentication/oauth2.go:38-40` — `SetValidateURIHandler` always returns nil, disabling redirect_uri host validation (open-redirect primitive; becomes code-exfiltration if a public client is ever added). Restore `DefaultValidateURI`. **Deferred** (behavior-risk to current flows; wants deliberate rollout).
- ⏸️ `internal/services/auth/handlers/authentication/oauth2.go:71-73` and `pkg/client/client.go:155-205` — only plain PKCE, optional, no S256; the client advertises `code_challenge_method=plain` with no challenge/verifier. Support/require S256 or drop the param. **Not addressed.**
...
- ⏸️ `internal/domain/identity/manager/user_data_manager.go:44,1079-1086` — QR PNG served with a `image/jpeg` data-URI prefix; also duplicates the injected `qrcodes.Builder`. **Not addressed.**

**Conventions / hygiene**
- ⏸️ `docs/writing_go.md` says "No `t.SkipNow()`", but 122 `t.SkipNow()` calls across 64 test files gate integration tests on `RunContainerTests`. Legitimate intent, but it conflicts with the stated convention — consider `t.Skip("...")` with a reason, or update the doc. **Not addressed.**
...
- 🟡 ~19 TODO/FIXME markers in hand-written code: `internal/services/identity/grpc/accounts.go:268` (the H9 auth gap) **removed**; `internal/authorization/mealplanning_permissions.go:161` (missing `clone.recipes` permission) **still open**.

---

## Remaining work (not addressed this pass)

Carry-over backlog, roughly in priority order:

- **RC4 / data-privacy export completeness** — pagination-to-exhaustion + the missing domains (comments, oauth, payments). The service is now reachable and ownership-checked, but exports still truncate at 50 rows/category.
- **H18 tail** — the vote-eligibility gate (needs the meal-plan/event IDs threaded into the manager signature). H10 is now fully closed: meal-list scoping and the recipe-option-selection scoping (`CheckMealPlanOptionBelongsToAccount` + `verifyMealPlanOptionAccess`) are both done.
- **H15 thaw tasks** — the nullable `belongs_to_recipe_prep_task` migration to re-enable the (currently disabled) feature.
- **Mediums:** M5 (archived recipe-steps join), M12 (membership converter), M13 (invitation token in logs), M26 (page-size truncation), M29 (duplicated DI containers / double migration run), M33 (MCP port).
- **`admin.go:50`** — an RC1-style `PrepareAndLogGRPCStatus(nil, …)` that turns a permission check into a `(nil, nil)` bypass. Should be promoted out of "lows" and fixed.
- **Identity-domain lows** — `SearchForUsers` counts, `todoRes`/`ConvertUserToUserCreationResponse` dead code, QR MIME prefix, nil-input guards, `Selections` dropped on option creation, `clone.recipes` permission, the `t.SkipNow` convention, and the meal.go dead update types.
- **Deferred security hardening** — M16 hashed secret storage, OAuth open-redirect validation, S256 PKCE.
- **M23 waitlist signups** — blocked on the product access-model decision above.

---

## Suggested sequencing for follow-up agents

1. **Retire RC1 first** (H1, H3, H4, H5, H12, H13): audit `PrepareError`/`PrepareAndLogError` call sites on error branches, replace nil args with explicit errors, add the panic-recovery interceptor (H2). Small diffs, closes several highs and the crash vector. — ✅ done (plus one residual at `admin.go:50`).
2. **Sweep RC2 IDOR** (H6–H10, H14, M20–M25): one uniform change — session identity over request IDs, ownership checks on by-ID reads. Consider a shared helper so the pattern can't drift again. — 🟡 mostly done (H10 tail + M23 open).
3. **Wire RC3 validators** (H11) and fix the meal-planning correctness bugs (H15–H19). — ✅ done (H15/H18 tails open).
4. **Workers & shutdown** (H20–H22, M28–M31): finalization event routing, graceful drain, telemetry flush, idempotency. — ✅ done (M29 open).
5. **RC4 data-privacy completeness**, then the medium data-layer/transaction fixes (M1–M6), then lows. — RC4 open; M1–M4, M6 done (M5 open).
