...
#     protoc's .pb.go and moq's *_mock.go — reporting ~1400 diagnostics it would not fix,
#     leaving every file byte-identical while still exiting non-zero. betteralign skips
#     generated and test files by default, which matches the repo rule that generated code
...
for ((pass = 1; pass <= MAX_PASSES; pass++)); do
  ...
  if go tool betteralign -apply ./... > /dev/null 2>&1; then
    break
  fi
